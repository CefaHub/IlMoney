...
public final class YamlBalanceStore implements BalanceStore {
    ...
    public YamlBalanceStore(File file) {
        this.file = file;
    ...
    @Override public void save() throws IOException { cfg.save(file); }

    @Override public double get(UUID id) { return Math.max(0, cfg.getDouble("balances." + id, 0D)); }

    @Override public void set(UUID id, double amount) { cfg.set("balances." + id, Math.max(0, amount)); }

    @Override public void setName(UUID id, String name) { if (name != null) cfg.set("names." + id, name); }
